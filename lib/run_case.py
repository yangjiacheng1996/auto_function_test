...
'''
log_file = os.path.join(varpool.result_dir, "result.log")
...
def load_case_list():
    with open(os.path.join(varpool.project_dir, "result", "case.json"), encoding="utf-8") as j:
        return list(json.load(j))


def load_vars():
    with open(os.path.join(varpool.project_dir, "result", "vars.json"), encoding="utf-8") as j:
        vars = json.load(j)
        varpool.update(vars)
        return varpool
...
def test_case(case):
    ...
    allure.dynamic.suite(varpool.testplan_filename)
    # load case
    allure.dynamic.title("Case: %s" % case["case"]["case_name"])
    for step in case["case"]["steps"]:
        ...
        function_obj = get_function(os.path.join(varpool.project_dir, "case", script), function_name)
